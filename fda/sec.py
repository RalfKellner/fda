...
def cik_company_tickers(user_mail: str) -> pd.DataFrame:
...
def get_10_form_filing(cik: str, user_mail: str, accession_number: str, primary_doc: str, min_length_extract_from_body: int, raw_html: bool = False) -> str:
    ...
    user_mail: the mail which is used as a header information which is demanded by the SEC
    min_length_extract_from_body: Some of the SEC sites cotain information in paragraph tags, others don't. If a text has less than this value, the site does 
    ...
    if raw_html:
        ...
        text = re.sub("\s+", " ", text)
        if len(text) < min_length_extract_from_body:
...
def get_8K_filing_and_exhibits(cik: str, user_mail: str, accession_number: str, min_length_extract_from_body: int, separator_character: str = "-;-") -> str:
